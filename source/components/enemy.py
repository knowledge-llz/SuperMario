...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, direction, name, frame_rects):
        ...
        self.timer = 0
        self.x_vel = -1 * C.ENEMY_MULTI if self.direction == 0 else C.ENEMY_SPEED
        self.y_vel = 0
    ...
    def check_x_collision(self, level):
        ...
        if sprite:
            if sprite.rect.right < self.rect.right and self.direction == 0:
                self.direction = 1
                self.rect.left = sprite.rect.right
            elif sprite.rect.right > self.rect.right and self.direction == 1:
                self.direction = 0
...
class Goomba(Enemy):
    ...
    def trampled(self, level):
        ...
        if self.death_timer == 0:
            self.current_time = self.death_timer
