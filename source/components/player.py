...
class Player(pygame.sprite.Sprite):
        ...
        for group, group_frame_rects in frame_rects.items():
            for frame_rect in group_frame_rects:
                right_image = tools.get_image(sheet, frame_rect['x'], frame_rect['y'],
                                              frame_rect['width'], frame_rect['height'], (0, 0, 0), C.PLAY_MULTI)
    ...
    def small_to_big(self, keys):
        ...
        if self.transition_timer == 0:
            ...
            self.transition_timer = self.current_time
            frames, idx = frames_and_idx[sizes[self.idx]]
            self.change_play_image(frames, idx)
            self.idx += 1
            if self.idx == len(sizes):
                self.transition_timer = 0
                self.state = 'walk'
    ...
    def change_play_image(self, frames, idx):
        self.frame_index = idx
        if self.face_right:
            self.right_frames = frames[0]
            self.image = self.right_frames[self.frame_index]
        else:
            self.left_frames = frames[0]
            self.image = self.left_frames[self.frame_index]
    ...
    def is_hurt_immue(self):
        if self.hurt_immue:
            if self.hurt_immue_timer == 0:
                self.hurt_immue_timer = self.current_time
                self.blank_image = pygame.Surface((1, 1))
            elif self.current_time - self.hurt_immue_timer > 2500:
                self.hurt_immue = False
                self.hurt_immue_timer = 0
            else:
                if (self.current_time - self.hurt_immue_timer) % 100 < 50:
